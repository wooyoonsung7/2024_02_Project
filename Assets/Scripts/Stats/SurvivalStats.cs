...
{
    [Header("Huger Settings")]
    public float maxHuger = 100;            //최대 허기량
    ...
    public float craftingDamage = 3f;           //제작시 우주복 데미지
    ...
    void Update()
    {
        ...
        if (hungerTimer >= 1.0f)
        {
            currentHunger = Mathf.Max(currentHunger - hungerDecreaseRate);
            hungerTimer = 0;
    ...
    public void EatFood(float amount)
    {
        ...
        currentHunger = Mathf.Min(maxHuger, currentHunger + amount);            //maxHuger 값을 넘기지 않기 위해
    ...
    public void ReqairSuit(float amount)
    {
    ...
    private void CheckDeath()                                //플레이ㅓ 사마 처리 체크 함수
    {
    ...
    {
        return (currentHunger / maxHuger) * 100;
    }
